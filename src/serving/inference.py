...
try:
    # Read feature columns from the model directory or the artifacts directory
    feature_file = os.path.join(MODEL_DIR, "feature_columns.txt")
    if not os.path.exists(feature_file):
        # Fallback to looking in the parent artifacts directory
        parent_dir = os.path.dirname(MODEL_DIR)
        feature_file = os.path.join(parent_dir, "feature_columns.txt")
        if not os.path.exists(feature_file):
            print(f"⚠️ Warning: Could not find feature_columns.txt in {MODEL_DIR} or {parent_dir}")
            # Try searching up the tree or looking in all recent mlruns
            import glob
            files = glob.glob("./mlruns/**/artifacts/feature_columns.txt", recursive=True)
            if files:
                # Sort by modification time to get the latest
                files.sort(key=os.path.getmtime, reverse=True)
                feature_file = files[0]
                print(f"✅ Found latest feature_columns.txt at: {feature_file}")
            else:
                raise Exception(f"Failed to load feature columns: File not found")
